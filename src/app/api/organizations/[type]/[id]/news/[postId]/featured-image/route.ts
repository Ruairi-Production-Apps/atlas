...
// POST - Upload featured image for existing news post
export async function POST(
    ...
    // Check permissions (same as news PATCH)
    ...
    try {
        ...
        const fileName = `${type}/${id}/${postId}/${Date.now()}.${fileExt}`
        ...
        if (post?.featured_image_url) {
            const oldUrlParts = post.featured_image_url.split('/news-images/')
...
export async function DELETE(
    ...
    try {
        ...
        if (fetchError || !post?.featured_image_url) {
        ...
        // Update news post to remove image URL
        const { error: updateError } = await supabase
            .from('news_posts')
